...
def scenario_all_healthy() -> FakeScenario:
    ...
    biceps_curl:     avg_difficulty ≈ 0.290  →  personal_score = 0.290 (LOW)

    Expected order: shoulder_press > push_up > biceps_curl
    ...
    return FakeScenario(
        ...
        history=[
            ...
            _rec(uid, "biceps_curl",     0.28, 0.92, [], health, 12),
            _rec(uid, "biceps_curl",     0.30, 0.90, [], health, 5),
        ],
        feedbacks=[],
...
def scenario_core_pain() -> FakeScenario:
    ...
    biceps_curl:     avg_difficulty = 0.280  →  moderate
    shoulder_press: avg_difficulty = 0.815  →  penalised (>0.75 threshold)

    Expected order: push_up ≈ biceps_curl >> shoulder_press
    ...
    return FakeScenario(
        ...
        history=[
            ...
            _rec(uid, "biceps_curl",     0.27, 0.93, [], health, 6),
            _rec(uid, "biceps_curl",     0.29, 0.91, [], health, 2),
        ],
        feedbacks=[],
...
def scenario_upper_injured() -> FakeScenario:
    ...
    biceps_curl:  difficulty=0.92, form=0.45  →  near-zero (diff≥0.90 on injured region)
    push_up:     difficulty=0.30, form=0.90  →  good (easy + great form)
    tricep_dip:  no history                  →  0.30 (no history, unsafe default)

    Expected: push_up >> tricep_dip >> biceps_curl
    """
    ...
    return FakeScenario(
        ...
        target_region=BodyRegion.UPPER,
        history=[
            _rec(uid, "biceps_curl", 0.92, 0.45, ["joint_misalignment"], health, 5),
            _rec(uid, "push_up",    0.30, 0.90, [], health, 7),
            _rec(uid, "push_up",    0.28, 0.92, [], health, 3),
        ],
        feedbacks=[],
        description="TARGET_UNHEALTHY: push_up (safe) should score far above biceps_curl (risky)",
    )
